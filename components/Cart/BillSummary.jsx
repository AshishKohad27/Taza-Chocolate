...
import stylesP from "./payment-method.module.css";

export default function BillSummary({ data }) {
    ...
    <Box h="auto" className={styles.billSummary}>
      ...
      <Box className={stylesP.paymentMethodCss}>
        <Box mt="20px">
          ...
          <Heading as="h1" className={styles.charges}>
            {data.length > 0 ? (
              <>
                <span className={styles.crossSaving}>
                  {" "}
                  ₹{" "}
                  {data
                    .reduce((a, b) => a + b.mrp * b.quantity, 0)
                    .toFixed(2)}
                </span>
                <span>
                  {" "}
                  ₹{" "}
                  {data
                    .reduce((a, b) => a + b.bar * b.quantity, 0)
                    .toFixed(2)}
                </span>
              </>
            ) : (
      ...
      <Box>
        ...
        <Heading as="h1">
          {data.length > 0
            ? ` ₹ ${(
                data.reduce((a, b) => a + b.bar * b.quantity, 0) +
                100
              ).toFixed(2)}`
            : 0}
        </Heading>
